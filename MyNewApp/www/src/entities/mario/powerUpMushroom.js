...
export class PowerUpMushRoom {
    ...
    // Horizontal movement
    this.position.x += this.velocity.x * this.direction * dt * 60;
    ...
    for (const brick of this.game.bricks) {
        ...
        if ((hittingLeftWall || hittingRightWall) && this.onGround) {
            this.direction *= -1;
            this.position.x += this.direction * Math.abs(this.velocity.x) * dt * 60; // push away to prevent sticking
        }
    }
    ...
    draw(context, stage) {
        ...
        if(gameState.debug.entities) context.strokeRect(box.x - stage.x, box.y - stage.y, box.width, box.height);
    }
}