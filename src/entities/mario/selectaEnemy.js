...
import { IceCream } from './entity/iceCream.js';
import { ScoreText } from './scoreText.js';

export class SelectaEnemy {
    constructor(game, x, y, speed = 0.5, direction = 1) {
        ...
        this.position = { x: x, y: y };
         ...
         // --- Projectile / attack ---
        this.attackCooldown = Math.random() * 3 + 2; // random interval 2-5s
        this.attackTimer = 0;
        ...
        this.frames = new Map([
             ['idle', [
               [[405, 304, 89, 99], [45, 97], { push: [-45, -50, 89, 50], hurt:  [-45, -50, 89, 50] }],
            ]],           
             ['dead', [
                [[405, 304, 89, 99], [45, 97], { push: [0, 0, 0, 0], hurt: [0, 0, 0, 0] }],
            ]],        
        ]);
...
handleIdleState(time) {
    ...
    // --- Throw IceCream projectile ---
    this.attackTimer += time.secondsPassed; // make sure 'time' is passed from update()
    if (this.attackTimer >= this.attackCooldown) {
        this.throwIceCream();
        this.attackTimer = 0;
        this.attackCooldown = Math.random() * 3 + 2; // reset cooldown
    }
}

throwIceCream() {
    // spawn IceCream slightly above enemy
   const ice = new IceCream(
    ...
    ice.gravity = 0.5;
    ice.bounceDamping = 1;
    console.log("Throwing IceCream from SelectaEnemy at", ice.position);
